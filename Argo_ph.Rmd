...
```{r Graph2 MOOSE 130, echo=FALSE, warning= FALSE,out.width='100%'}
### Plot pH with error bars ###
d <- filter(data, Cruise=="Moose130") # discrete
ad <- lovapm016c %>% #Argo
  dplyr::mutate(adate = as_date(adatetime)) #%>%
  #dplyr::filter(!is.na(apH) & apH < 9 & (adate == "2017-12-04" | adate == "2017-12-05"))
ggplot() + 
...
#Now spec pH
  geom_point(data = d, 
             aes(y = Depth, x = pH_Tinsitu, color="Moose130"), size= 1) +
  geom_path(data = d, aes(y = Depth, x = pH_Tinsitu, color="Moose130")) +
  geom_errorbarh(data = d, 
                 aes(y = Depth, x = pH_Tinsitu, xmin = pH_Tinsitu - sd,
                     xmax = pH_Tinsitu + sd, color="Moose130"),
                 height=2, size=0.5) +
      scale_y_reverse()
```
