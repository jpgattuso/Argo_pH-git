...
- 130 on 2019-04-16. Samples have been collected at XXX by Eduardo Soto and Melek Golbol.


It also shows data obtained with Argo float lovapm016a.
...
```{r Argo data, echo=FALSE, message=FALSE, out.width='100%'}
#MOOSE 113 and 114
if (file.exists("./../data/lovapm016a.RData") == TRUE) {
  load("./../data/lovapm016a.RData")
  lovapm016a <- as_tibble(lovapm016a)
}
files <- curl("http://poteau:poteau@www.oao.obs-vlfr.fr/BD_FLOAT/NETCDF/lovapm016a/liste_all", open="r")
files <- readLines(files)
for (i in 1:length(files)){
  loc_file_name <-  str_extract(files[[i]], "out(.+)nc")
  loc_file_path <-  paste0("./../data/argo_archive/", loc_file_name)
rem_file_name <- paste0("http://poteau:poteau@www.oao.obs-vlfr.fr/BD_FLOAT/NETCDF/lovapm016a/", loc_file_name)

if (file.exists(paste0("./../data/argo_archive/", loc_file_name)) == FALSE){
  print(loc_file_name)
...
if (file.exists("./../data/lovapm016a.RData") == TRUE) {
  lovapm016a <- bind_rows(lovapm016a, adata)
} else {
  lovapm016a <- adata
  }
}
}
lovapm016a <- lovapm016a %>%
  dplyr::distinct(lovapm016a) %>% # remove duplicates if any
  dplyr::arrange(adatetime) # re-order

save(lovapm016a, file = "./../data/lovapm016a.RData")

#MOOSE 120
if (file.exists("./../data/lovapm016b.RData") == TRUE) {
  load("./../data/lovapm016b.RData")
  lovapm016b <- as_tibble(lovapm016b)
}
files <- curl("http://poteau:poteau@www.oao.obs-vlfr.fr/BD_FLOAT/NETCDF/lovapm016b/liste_all", open="r")
files <- readLines(files)
for (i in 1:10){
  loc_file_name <-  str_extract(files[[i]], "out(.+)nc")
  loc_file_path <-  paste0("./../data/argo_archive/", loc_file_name)
rem_file_name <- paste0("http://poteau:poteau@www.oao.obs-vlfr.fr/BD_FLOAT/NETCDF/lovapm016b/", loc_file_name)
...
if (file.exists(paste0("./../data/argo_archive/", loc_file_name)) == FALSE){
  ...
  adata <- tibble(adatetime, aTinsitu, aSal, aProf, apH)
if (file.exists("./../data/lovapm016b.RData") == TRUE) {
  lovapm016b <- bind_rows(lovapm016b, adata)
} else {
  lovapm016b <- adata
  }
}
}
lovapm016b <- lovapm016b %>%
  dplyr::distinct(lovapm016b) %>% # remove duplicates if any
  dplyr::arrange(adatetime) # re-order

save(lovapm016b, file = "./../data/lovapm016b.RData")
```

```{r read lab data, echo=FALSE, message=FALSE, out.width='100%'}
d <- read_delim("../data/Dyfamed.csv", col_names=TRUE, delim = ";")
d$date <- dmy(d$date)  
